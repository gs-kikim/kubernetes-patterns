...
# ImageBuilder 패턴 전체 테스트 실행 스크립트
# Chapter 30: 컨테이너 이미지 빌드 — Kaniko, BuildKit
...
echo "========================================"
echo "ImageBuilder 패턴 전체 테스트"
echo "Kubernetes Patterns — Chapter 30"
echo "========================================"
echo ""
echo "테스트 시작 시간: $(date)"
echo ""
...
# 테스트 3: 블로그 핵심 개념 검증
run_test "test-imagebuilder-concepts.sh" "Test 3: 블로그 핵심 개념 검증"

# 테스트 4: 클러스터 배포 테스트 (선택적)
echo ""
echo -e "${YELLOW}테스트 4: 클러스터 배포 테스트를 실행하시겠습니까?${NC}"
echo "이 테스트는 minikube + registry addon이 필요합니다."
echo "Kaniko 및 BuildKit으로 이미지를 빌드하고 검증합니다 (약 5~10분 소요)."
read -p "실행? (y/N): " -n 1 -r
echo ""
if [[ $REPLY =~ ^[Yy]$ ]]; then
    run_test "test-cluster-deploy.sh" "Test 4: 클러스터 배포 테스트 (이미지 빌드 검증)"
else
